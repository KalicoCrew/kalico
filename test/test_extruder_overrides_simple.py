...
def test_extruder_overrides_danger_options_values(
    ...
    """Test that danger_options override values are loaded correctly"""
    ...
    with PrinterShim(start_args) as printer:
        ...
        # Test that override values are loaded correctly
        assert danger_options.override_pressure_advance_min == 0.0
        assert danger_options.override_pressure_advance_max == 2.0
        assert danger_options.override_pressure_advance_smooth_time_min == 0.0
        assert danger_options.override_pressure_advance_smooth_time_max == 1.0
        assert danger_options.override_max_extrude_only_distance_min == 0.0
        assert danger_options.override_max_extrude_only_distance_max == 200.0
        assert danger_options.override_instantaneous_corner_velocity_min == 0.0
        assert danger_options.override_instantaneous_corner_velocity_max == 10.0
        assert danger_options.override_max_extrude_cross_section_min == 0.0
        assert danger_options.override_max_extrude_cross_section_max == 100.0
        assert danger_options.override_max_extrude_only_velocity_min == 0.0
        assert danger_options.override_max_extrude_only_velocity_max == 2000.0
        assert danger_options.override_max_extrude_only_accel_min == 0.0
...
def test_extruder_overrides_defaults(
    ...
    """Test that default override values work without [danger_options] section"""
    start_args = {"config_file": str(config_root / "printer.cfg")}
    ...
    with PrinterShim(start_args) as printer:
        ...
        # Test that default values are applied when no overrides are specified
        assert danger_options.override_pressure_advance_min == 0.0
...
def test_extruder_overrides_restrictive(
    ...
    """Test that restrictive override values work correctly"""
    start_args = {"config_file": str(config_root / "printer.cfg")}
    ...
    with PrinterShim(start_args) as printer:
        ...
        # Test that restrictive values are loaded correctly
        assert danger_options.override_pressure_advance_min == 0.0
        assert danger_options.override_pressure_advance_max == 2.0
        assert danger_options.override_pressure_advance_smooth_time_min == 0.0
        assert danger_options.override_pressure_advance_smooth_time_max == 1.0
...
def test_extruder_config_validation_with_overrides(
    ...
    """Test that extruder config validation works with overrides"""
    start_args = {"config_file": str(config_root / "printer.cfg")}
    
    with PrinterShim(start_args) as printer:
        config = printer.load_config()
        
        # Test that extruder config values within override bounds are accepted
        # This test will fail if the overrides don't work, because the config
        # has values that would normally be outside the default limits
        extruder_section = config.getsection("extruder")
        
        # These values should be accepted because of the overrides
