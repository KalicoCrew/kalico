...
def eval_nurbs(degree, control_points, knots, weights, u):
    """Evaluate a NURBS curve at parameter u; returns (x, y, z)."""
    ...
    for i in range(degree + 1):
        ...
        sx += wn * cp[0]
        sy += wn * cp[1]
        sz += (wn * cp[2]) if len(cp) > 2 else 0.0
        sw += wn
    if sw == 0.0:
        return (0.0, 0.0, 0.0)
...
def load_fixture(io, fixture, slot=0, timeout=3.0):
    ...
    for cp in fixture["control_points"]:
        cps.append(cp[0])
        cps.append(cp[1])
        cps.append(cp[2] if len(cp) > 2 else 0.0)
