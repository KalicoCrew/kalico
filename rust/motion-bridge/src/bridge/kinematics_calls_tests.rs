...
fn corexy_forward_delta_only_slot0_moves_when_dx_eq_dy() {
    ...
    assert_eq!(slots[0].0, AXIS_X as u8, "moving slot should be AXIS_X (A motor)");
...
fn corexy_forward_motor_positions_xyz() {
    ...
    assert!((m[AXIS_X] - 4.0).abs() < 1e-9, "slot AXIS_X (A) = 4, got {}", m[AXIS_X]);
    assert!((m[AXIS_Y] - 2.0).abs() < 1e-9, "slot AXIS_Y (B) = 2, got {}", m[AXIS_Y]);
}
