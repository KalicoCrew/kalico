...
runtime_tick_enable(void)
{
    ...
    // The pre-2026-05-19 path armed TIM5 immediately on axis configure,
    // even before any segment existed — so the ISR fired at 40 kHz writing
    // zero-delta XDIRECT to SPI3 for the entire idle period, starving the
    // USB CDC pump and eventually causing "No such device" disconnects under
    // sustained load.
