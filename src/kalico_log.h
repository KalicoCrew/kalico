...
// canonical table in rust/runtime/src/log_codes.rs — keep in sync. Only the
// C-side emits (boot marker + the drain's ring-overflow report + crash
// forensics) need these mirrors; Rust emit sites use the Rust constants.
#define KALICO_LOG_SUBSYS_RUNTIME 0
// Diag play-by-play: the prior-boot diag event ring is replayed under this
// subsystem with event-code == the DIAG_EV_* tag (1:1, see fault_handler).
