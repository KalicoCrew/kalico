...
enum {
    ...
    SF_SINGLE_SCHED=1<<4, SF_OPTIMIZED_PATH=1<<5, SF_HAVE_ADD=1<<6, SF_HIGH_PREC_STEP=1<<7
};
...
stepper_load_next(struct stepper *s)
{
    ...
    if (HAVE_OPTIMIZED_PATH && s->flags & SF_OPTIMIZED_PATH) {
        ...
        if (HAVE_AVR_OPTIMIZATION)
            s->flags = (move_have_add ? s->flags | SF_HAVE_ADD
                        : s->flags & ~SF_HAVE_ADD);
        s->count = move_count;
    } else {
...
command_queue_step(uint32_t *args)
{
    ...
    m->first_interval = args[1];
    m->add = args[3];
    m->interval = m->first_interval + m->add;
    m->flags = 0;
    if (HAVE_AVR_OPTIMIZATION) {
