...
# and damping ratio
STEP_SIMULATION_RESONANCE_FREQ = 50.0
...
def gen_shaper_response(shaper, shaper_freq, estimate_shaper):
    ...
    freq_start, freq_end = find_shaper_plot_range(
        lambda freq: estimate_shaper(np, shaper, DAMPING_RATIOS[0], [freq]),
        shaper_freq,
        vib_tol=0.25,
    )
    ...
    for i in range(n_dr):
        response[:, i] = estimate_shaper(np, shaper, DAMPING_RATIOS[i], freqs)
...
def gen_shaped_step_function(shaper, freq, damping_ratio):
    ...
    t_start = T[0] - 0.5 * t_s
    t_end = T[-1] + 1.5 * max(1.0 / freq, t_s)
    dt = 0.01 * min(t_s, 1.0 / freq)
    time = np.arange(t_start, t_end, dt)
    time_t = time[np.newaxis].T

    step = np.zeros(time.shape)
    step[time >= 0.0] = 1.0
    commanded = np.sum(A * np.where(time_t - T >= 0, 1.0, 0.0), axis=-1) * inv_D
    response = np.zeros(shape=(time.shape))
    for i in range(n):
        response += (
            A[i]
            * shaper_calibrate.step_response(
                np, time - T[i], omega, damping_ratio
            )[0, :]
        )
    response *= inv_D
    velocity = np.insert(
        (response[1:] - response[:-1]) / (2 * math.pi * freq * dt), 0, 0.0
    )
    legend = [
        "step",
        "shaper commanded",
...
def gen_smoothed_step_function(smoother, freq, damping_ratio):
    ...
    t_start = -t_sm - t_offs
    t_end = hst - t_offs + 1.5 * max(1.0 / freq, t_sm)
    time = np.arange(t_start, t_end, dt)
    time_t = time[np.newaxis].T
    w_dt = w * dt
    step = np.zeros(time.shape)
    step[time >= 0.0] = 1.0
    commanded = np.sum(
        w_dt * np.where(time_t - tau + t_offs >= 0, 1.0, 0.0), axis=-1
    )
...
def main():
    ...
    fig = plot_shaper(
        options.shaper, options.shaper_freq, options.freq, options.damping_ratio
    )
