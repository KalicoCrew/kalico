...
class FatFS:
    ...
    def _fatfs_cb_disk_ioctl(self, cmd, buff):
        ...
        logging.debug(
            "flash_sdcard: Received IOCTL command %s" % (ioctl_cmds[cmd])
        )
        return 0
    ...
    def mount(self, print_func=logging.info):
        ...
        if ret == 0:
            ...
            raise OSError(
                "flash_sdcard: failed to mount SD Card, returned %s"
                % (FRESULT[ret])
            )
    ...
    def get_disk_info(self):
        ...
        if ret != 0:
            logging.info(
                "flash_sdcard: Failed to retreive disk info: %s"
                % (FRESULT[ret],)
            )
            return {}
        return {
...
class SDCardFile:
    ...
    def open(self):
        ...
        if self.fhdl == self.ffi_main.NULL:
            self.fhdl = None
            raise OSError(
                "flash_sdcard: Could not open file '%s':" % (self.path)
            )
    ...
    def read(self, length=None):
        ...
        while True:
            ...
            if bytes_read < 0:
                raise OSError(
                    "flash_sdcard: Error Reading file '%s'" % (self.path)
                )
...
class SDCardSPI:
    ...
    def init_sd(self):
        with self.mutex:
            ...
            if resp and resp[0] & (1 << 2):
                ...
                raise OSError(
                    "flash_sdcard: Invalid CMD8 response: %s" % (repr(resp))
                )
            if self.enable_crc:
            ...
            if self.sd_version == 2:
                ...
                if not self._check_command(
                    ...
                    raise OSError(
                        "flash_sdcard: SD Card did not come"
            ...
            if len(resp) == 5:
                if self.sd_version == 1 and resp[0] == 1:
                    ...
                    raise OSError(
                        "flash_sdcard: READ_OCR Error 0x%X" % (resp[0],)
                    )
            else:
    ...
    def _process_cid_reg(self):
        ...
        cid["product_revision"] = (
            str(reg[8] >> 4 & 0xFF) + "." + str(reg[8] & 0xFF)
        )
        ...
        if crc != reg[15]:
            raise OSError(
                "flash_sdcard: CID crc mismatch: 0x%02X, recd: 0x%02X"
                % (crc, reg[15])
            )
    ...
    def _process_csd_reg(self):
        ...
        if csd_type == 0:
            ...
            c_size = (
                ((reg[6] & 0x3) << 10) | (reg[7] << 2) | ((reg[8] >> 6) & 0x3)
            )
        ...
        if crc != reg[15]:
            raise OSError(
                "flash_sdcard: CSD crc mismatch: 0x%02X, recd: 0x%02X"
                % (crc, reg[15])
            )
...
class MCUConnection:
    ...
    def verify_flash(self, req_chksm, old_dictionary, req_dictionary):
        ...
        if not validation_passed:
            cur_fw_path = self.board_config.get(
                "current_firmware_path", "FIRMWARE.CUR"
            )
            try:
...
class SPIFlash:
    def __init__(self, args):
        ...
        if not os.path.isfile(args["klipper_bin_path"]):
            raise SPIFlashError(
                "Unable to locate klipper binary at '%s'"
