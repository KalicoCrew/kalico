...
class MCUSentStream:
    ...
    def parse_line(self, line_num, line):
        ...
        if m is not None:
            ...
            seq = (
                self.mcu.shutdown_seq + int(m.group("count")) - self.send_count
            )
...
class GCodeStream:
    ...
    def handle_gcode_state(self, line):
        ...
        lp = kv["last_position"]
        out.append(
            "G1 X%f Y%f Z%f F%f" % (lp[0], lp[1], lp[2], kv["speed"] * 60.0)
        )
        bp = kv["base_position"]
        if bp[:3] != [0.0, 0.0, 0.0]:
...
trapq_r = re.compile(
    r"^Dumping trapq '(?P<name>[^']*)' " + count_s + r" moves:$"
)
...
class StatsStream:
    ...
    def parse_line(self, line_num, line):
        ...
        if m is not None:
            return True, StepperStream(
                m.group("name"), m.group("mcu"), self.mcus
            )
    ...
    def get_lines(self):
        ...
        for i, (ts, line_num, line) in enumerate(self.stats_stream):
            if ts is not None:
                ...
                line_num >= self.shutdown_line_num and last_ts <= max_stream_ts
            ):
