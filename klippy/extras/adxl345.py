...
class ADXLCommandHelper:
    ...
    def cmd_ACCELEROMETER_QUERY(self, gcmd):
        ...
        gcmd.respond_info(
            "adxl345 values (x, y, z): %.6f, %.6f, %.6f" % (accel_x, accel_y, accel_z)
        )
...
class ADXL345:
    def __init__(self, config):
        ...
        mcu.add_config_cmd(
            "query_adxl345 oid=%d clock=0 rest_ticks=0" % (oid,), on_restart=True
        )
        ...
        wh.register_mux_endpoint(
            "adxl345/dump_adxl345", "sensor", self.name, self._handle_dump_adxl345
        )
    ...
    def _update_clock(self, minclock=0):
        ...
        for retry in range(5):
            params = self.query_adxl345_status_cmd.send([self.oid], minclock=minclock)
    ...
    def _start_measurements(self):
        ...
        self.query_adxl345_cmd.send([self.oid, reqclock, rest_ticks], reqclock=reqclock)
