...
class ManualStateHelper:
    ...
    cmd_SET_PROBE_STATUS_help = (
        "Manually specify probe status, valid probe "
    ...
    def cmd_SET_PROBE_STATUS(self, gcmd):
        ...
        if state in probe_states:
            self.probe_state = next(
                k for k, v in enumerate(probe_states) if state == v
            )
        else:
...
class ProbeState:
    def __init__(self, config, aProbe):
        ...
        if not any(
            [self.check_open_attach, self.probe_sense_pin, self.dock_sense_pin]
        ):
    ...
    def cmd_GET_PROBE_STATUS(self, gcmd):
        ...
        state = next(
            v for k, v in enumerate(["UNKNOWN", "ATTACHED", "DOCKED"]) if s == k
        )
...
class DockableProbe:
    def __init__(self, config):
        ...
        self.allow_delayed_detach = config.getboolean(
        ...
        # Gcode Commands
        self.gcode.register_command("ATTACH_PROBE", self.cmd_ATTACH_PROBE)
        self.gcode.register_command("DETACH_PROBE", self.cmd_DETACH_PROBE)
        ...
        self.printer.register_event_handler("klippy:ready", self._handle_ready)
        self.printer.register_event_handler(
            "homing:home_rails_begin", self._handle_homing_rails_begin
        )
        self.printer.register_event_handler(
            "homing:home_rails_end", self._handle_homing_rails_end
        )
    ...
    def _build_config(self):
        ...
        endstops = [
            es for rail in kin.rails for es, name in rail.get_endstops()
        ]
        if self in endstops and self.printer.lookup_object("safe_z_home", None):
            e = (
                "{0}: cannot be used with safe_z_home if "
    ...
    def _handle_ready(self):
        ...
        if not self.safe_z_position:
            ...
            for rail in kin.rails:
                ...
                if hi.positive_dir:
                    mid = (hi.position_endstop - rmin) * 0.5
                else:
                    mid = rmax * 0.5
        ...
        if self.dock_fixed_z:
            ...
            if "z" in self._last_homed:
                if tpos[2] < dock_z:
                    ...
                    self.toolhead.manual_move(
                        [None, None, dock_z], self.lift_speed
                    )
            else:
                self._force_z_hop()
        else:
            if "z" in self._last_homed:
                self.toolhead.manual_move(
                    [None, None, self.dock_position[2]], self.lift_speed
                )
            else:
    ...
    def _force_z_hop(self):
        ...
        if self.last_z != this_z:
            ...
            self.toolhead.manual_move([None, None, dock_z], self.lift_speed)
    ...
    def detach_probe(self, return_pos=None):
        ...
        retry = 0
        while (
            self.get_probe_state() != PROBE_DOCKED and retry < self.dock_retries
        ):
            self._do_detach()
            retry += 1
            ...
            if any(a in ["x", "y"] for a in homing_axes):
                ...
                if "xy" in self._last_homed:
                    if self._check_distance(
                        self.dock_position, self.approach_distance
                    ):
                        self._align_to_vector(self.dock_angle)
                        self._move_to_vector(self.dock_angle)
            if "z" in homing_axes:
                if self.is_z_endstop:
                    ...
                    if "xy" in self._last_homed and not self.z_homes_positive:
    ...
    def multi_probe_begin(self):
        ...
        if not self._last_homed:
            curtime = self.printer.get_reactor().monotonic()
            self._last_homed = self.toolhead.get_status(curtime)["homed_axes"]
        if self.is_z_endstop and self.dock_fixed_z and "xy" in self._last_homed:
    ...
    def multi_probe_end(self):
        ...
        if pos[2] < retract_z:
            self.toolhead.manual_move([None, None, retract_z], self.lift_speed)
