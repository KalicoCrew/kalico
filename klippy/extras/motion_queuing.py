...
class PrinterMotionQueuing:
    ...
    def setup_mcu_movequeue(self, mcu, serialqueue, move_count):
        ...
        ss = ffi_main.gc(
            ffi_lib.steppersync_alloc(
                serialqueue, stepqueues, len(stepqueues), move_count
            ),
    ...
    def _advance_flush_time(self, want_flush_time, want_step_gen_time=0.0):
        ...
        if want_flush_time > flush_time + 10.0 * MOVE_BATCH_TIME:
        ...
        while 1:
            ...
            want_sg_wave = min(
                flush_time + STEPCOMPRESS_FLUSH_TIME, want_step_gen_time
            )
            step_gen_time = max(
                self.last_step_gen_time, want_sg_wave, flush_time
            )
