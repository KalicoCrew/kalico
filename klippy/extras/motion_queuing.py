...
class PrinterMotionQueuing:
    def __init__(self, config):
        ...
        self.steppersyncmgr = ffi_main.gc(
            ffi_lib.steppersyncmgr_alloc(), ffi_lib.steppersyncmgr_free
        )
        self.stepcompress = []
        self.steppersyncs = []
    ...
    def stats(self, eventtime):
        ...
        self.clear_history_time = max(0.0, est_print_time - MOVE_HISTORY_EXPIRE)
        return False, ""
    ...
    def _advance_flush_time(self, want_flush_time, want_step_gen_time=0.0):
        ...
        step_gen_time = max(
            want_step_gen_time, self.last_step_gen_time, flush_time
        )
        ...
        if not self.can_pause:
            clear_history_time = max(0.0, trapq_free_time - MOVE_HISTORY_EXPIRE)
        # Generate stepper movement and transmit
        ret = self.steppersyncmgr_gen_steps(
            self.steppersyncmgr, flush_time, step_gen_time, clear_history_time
        )
        if ret:
    ...
    def _await_flush_time(self, want_flush_time):
        while 1:
            ...
            if wait <= 0.0:
                return
            self.reactor.pause(systime + min(1.0, wait))
    ...
    def _flush_handler(self, eventtime):
        try:
            est_print_time = self.mcu.estimated_print_time(eventtime)
            aggr_sg_time = self.need_step_gen_time - 2.0 * self.kin_flush_delay
            if self.last_step_gen_time < aggr_sg_time:
                ...
                if want_sg_time > self.last_step_gen_time:
                    self._advance_flush_time(0.0, want_sg_time)
            else:
        ...
        try:
            ...
            while self.last_step_gen_time < faux_time:
                ...
                if flush_count > 100.0:
                    target = faux_time
                self._advance_flush_time(0.0, target)
                flush_count += 1
            if flush_count:
