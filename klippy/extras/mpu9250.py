...
class MPU9250:
    ...
    def _build_config(self):
        ...
        self.mcu.add_config_cmd(
            "query_mpu9250 oid=%d clock=0 rest_ticks=0" % (self.oid,),
            on_restart=True,
        )
    ...
    def _update_clock(self, minclock=0):
        ...
        for retry in range(5):
            params = self.query_mpu9250_status_cmd.send(
    ...
    def _start_measurements(self):
        ...
        time.sleep(20.0 / 1000)  # wait for accelerometer chip wake up
        ...
        rest_ticks = self.mcu.seconds_to_clock(1.0 / self.data_rate)
        self.query_rate = self.data_rate
        self.query_mpu9250_cmd.send(
    ...
    def _finish_measurements(self):
        if not self.is_measuring():
            return
        # Halt bulk reading
        _ = self.query_mpu9250_end_cmd.send([self.oid, 0, 0])
        self.query_rate = 0
        with self.lock:
            self.raw_samples = []
