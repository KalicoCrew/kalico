...
class InputShaperParams:
    def __init__(self, suffix, config):
        ...
        self.damping_ratio = config.getfloat(
            "damping_ratio_" + suffix,
            shaper_defs.DEFAULT_DAMPING_RATIO,
            minval=0.0,
            maxval=1.0,
        )
        self.shaper_freq = config.getfloat("shaper_freq_" + suffix, 0.0, minval=0.0)

    def update(self, gcmd):
        suffix = self.suffix.upper()
        self.damping_ratio = gcmd.get_float(
            "DAMPING_RATIO_" + suffix, self.damping_ratio, minval=0.0, maxval=1.0
        )
        self.shaper_freq = gcmd.get_float(
            "SHAPER_FREQ_" + suffix, self.shaper_freq, minval=0.0
        )
...
class AxisInputShaper:
    ...
    def matches_stepper(self, stepper_name):
        return (
            not self.config_suffix == DUAL_CARRIAGE_SHAPER_SUFFIX
    ...
    def report(self, gcmd):
        info = " ".join(
            [
                "%s_%s:%s" % (key, self.config_suffix, value)
...
class InputShaper:
    def __init__(self, config):
        ...
        if config.has_section(DUAL_CARRIAGE_SECTION):
            ...
            dc_shaper = AxisInputShaper(dc_axis, config, DUAL_CARRIAGE_SHAPER_SUFFIX)
