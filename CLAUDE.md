...
7. [ ] **First-print MVP: end-to-end on G5-only live pipeline (uniform cubic Bézier), with junction-deviation between sharp corners, smooth-ZV/smooth-MZV pre-bake, β-medium shaper-aware TOPP-RA, math-exact time-reparameterization (T-A), per-axis-scalar MCU storage, E-follows-shaped-XY foundation. No PA, no other smooth-shaper families, no corner-blend finalization. File-based prints run legacy G-code through Step-13 compat normalization (file → file); terminal/macro G1/G2/G3 (e.g. PRINT_START) is converted live by the bridge via compat primitives before the reduce stage.** Decomposes into sub-projects:
   ...
   - [x] **7-C-io — production host I/O:** NAK retransmit, async event dispatch, reconnect recovery, `arm_all_mcus` request_id correlation, `ArmError::QualityGate` detail, corpus-replay infrastructure. Deterministic test battery in place: `Clock` trait + `tick_once()` reactor entrypoint + A1–A7 covering seq-wrap, NAK/RTO branches, AwaitingResponse three-layer GC, NAK+submit race, partial-frame TCP-style assembly, ArcSwap status monotonicity, clock-sync drift over 24 virtual hours. (Spec: `docs/superpowers/specs/2026-04-30-step-7c-io-design.md`; tail spec: `docs/superpowers/specs/2026-05-01-step-7c-io-tail-design.md`; plans: `docs/superpowers/plans/2026-04-30-step-7c-io.md`, `docs/superpowers/plans/2026-05-01-step-7c-io-tail.md`.) Renode sim-soak, canonical H723 capture corpus, `python-diff-test` retirement (gated on canonical captures per parent spec §4.13), 24h soak, USB-CDC fidelity, IWDG real-world pacing, and Surface-C cycle actuals all moved to Step 7-D.
   - [x] **7-C-bridge — klippy bridge:** Python ↔ Rust integration so existing Klipper configs route motion through kalico's planner. Renode Phase-2 gate passed 2026-05-02 (wire-level bridge contract, `G1 X10`/`G1 Z5` segment dispatch, MCU tick_counter=886 on Gate A).
