...
- **The planner reduce stage accepts G5 / G5.1 only; the rejection boundary is at reduce, not the lexer.** The `rust/gcode` lexer tokenizes legacy G0/G1/G2/G3 (compat and the bridge depend on it). File-based prints are normalized to G5-only before they reach reduce (Step-13 compat layer or the bridge's file-normalization path). Live terminal/macro G1/G2/G3 (e.g. PRINT_START) is converted by the bridge via `compat` primitives before invoking reduce. The `compat` crate has two callers: the offline Step-13 binary (file → file, the original framing) and the live bridge for terminal/macro G1/G2/G3 conversion. Both share the lexer in `rust/gcode` and the primitive functions in `compat::{collinear,arc,degree_elev}`. This collapses the live planner to a uniform cubic Bézier representation with no rational NURBS, no mixed-degree dispatch, no source-gcode-type special-cases. The compat layer subsumes what was originally framed as the "streaming/windowed/real-time-tolerance G1-sequence spline fitter — highest-risk item in the spec" — that streaming framing is no longer needed. As an offline pre-processor (and live-bridge primitive), the standard CNC literature (Tajima/Sencer 2016, Beudaert 2012, Goldapp 1991) applies directly. Kalico-aware slicers emit G5 directly and never invoke the compat layer. Risk: low. Critical-path: no longer.
...
**`compat` crate callers:** The `compat` crate has two callers: the offline Step-13 binary (file → file, the original framing) and the live bridge for terminal/macro G1/G2/G3 conversion. Both share the lexer in `rust/gcode` and the primitive functions in `compat::{collinear,arc,degree_elev}`.
