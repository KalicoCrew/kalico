...
> **The `rust/gcode` lexer** is shared infrastructure: it tokenizes any G-code (including G0/G1/G2/G3) — `compat`'s offline normalizer and file preprocessor both depend on this. Tokenization is not the rejection boundary.
>
> **The planner reduce stage** (`rust/geometry`'s reducer + `rust/temporal`, `rust/trajectory`) is the rejection boundary. It accepts only G5 / G5.1. Anything else reaching reduce is a hard error.
>
> **The bridge** (`rust/motion-bridge`) sits above the planner reduce stage. It accepts G1 / G2 / G3 from klippy's Python gcode handlers (terminal commands, gcode-macros) as *structured* motion intent (deltas + feedrate + arc params) — never as raw text — and converts to G5 control points using the existing `compat` crate's primitive API (`compat::collinear::to_collinear_g5`, `compat::arc::arc_to_g5`, `compat::degree_elev::elevate_g51_to_g5`) before submitting to the reduce stage. The bridge is not a planner crate; the reduce boundary still sees only G5/G5.1.
>
...
- `klippy/chelper/trdispatch.c` (and its `defs_trdispatch` cffi block in `klippy/chelper/__init__.py`) — multi-MCU trigger coordination for `mcu_trsync` (homing). Includes `serialqueue.h` directly and allocates `struct trdispatch_mcu` with `struct serialqueue *` / `struct command_queue *` pointers. Cannot survive `serialqueue.c` deletion. Replaced in Phase 4 (homing) by bridge-native trigger coordination: `bridge.arm_endstops()` (§3.3) is the architectural slot for this. Phase 1 stubs the Python-side `mcu.MCU_trsync` class to no-op (`G28` rejected with "homing not yet implemented") so klippy boots; Phase 4 lands the real Rust-side trigger coordination.
...
| **4. Homing + drip + endstops + transforms** | `arm_endstops` + `submit_drip_move`; **bridge-native multi-MCU trigger coordination replacing trdispatch** (architectural slot is the `EndstopArm` list passed to `arm_endstops` — Rust side coordinates trigger fan-out across heterogeneous MCUs); klippy `homing.py` patches; sensorless TMC (passthrough) + beacon Z (passthrough on beacon MCU); `mcu.MCU_trsync` un-stubbed, now backed by bridge-native coordination. `bed_mesh.py` transform works through `motion_toolhead`. `skew_correction.py` chained transform works. | `G28` succeeds in sim with stub endstop triggers; `BED_MESH_CALIBRATE` works against beacon scanning mode. |
...
- **CLAUDE.md** "G5 / G5.1 only" bullet — change "anything that reaches the planner is G5 or G5.1; anything else is rejected at the lexer/reduce boundary as a hard error" to "anything reaching the planner *reduce stage* is G5 or G5.1; anything else is rejected at reduce as a hard error. The `rust/gcode` lexer remains capable of tokenizing legacy G-code because `compat` (Step 13's normalizer) depends on it." Add the bridge-as-second-caller text per §1.4.
- **CLAUDE.md** Step 7 / Step 13 prose — clarify that Step 13's `compat` crate has two callers: the offline binary (file→file) and the live bridge (terminal/macro G1/G2/G3 conversion via the same primitive functions). Both depend on the shared lexer.
- **`docs/kalico-rewrite/dependency-graph.md`** Layer 1 G-code parser bullet — change "Legacy G0 / G1 / G2 / G3 are not handled by the live parser" to "Legacy G0 / G1 / G2 / G3 are not handled by the planner *reduce stage*; the lexer tokenizes them and `compat` consumes the tokens. The bridge above the reduce stage converts terminal/macro G1/G2/G3 via the `compat` crate's primitive API before invoking reduce. File-based prints are normalized once at print-start."
