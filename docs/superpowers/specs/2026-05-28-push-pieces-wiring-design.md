...
- **A late piece kills the print, and that's fine — the design's job is to never let it happen.** When `now − start_time > FAULT_TOLERANCE` (`engine.rs:654`), the MCU faults and the print aborts (via Klipper `shutdown()` from the foreground drain — *not* a clean instantaneous engine-level stop; one axis can be briefly off-course before the halt lands). The exact mechanism doesn't matter to this spec: a late piece is catastrophic and unrecoverable in-stream, full stop. Our architecture tolerates the messy ~1 ms of the halt itself; making the fault path cleaner (and propagating it across MCUs) is **not** this spec's responsibility. The only obligation here is the §5.1 budgets that keep underrun from occurring.
