...
- **A late piece halts the whole MCU — but via Klipper `shutdown()`, not an engine-level stepper stop, and ~1 ms later.** This is where the contract doc's intent (2026-05-27 spec §6: "Hard fault — halt all motion") and the *code* diverge; the code path actually is:
  1. ISR detects `now − start_time > FAULT_TOLERANCE` (2 ticks, ≈ tens of µs; `engine.rs:654-659`) and calls `raise_piece_start_in_past`, which **only** stores `last_error` + `fault_detail` on `SharedState` (`fault_helpers.rs:127-133`). It does **not** set `RuntimeStatus::Fault`. The `runtime_force_idle` fault guard (`engine.rs:557`) therefore never engages, and the ISR keeps iterating **all** axes (`engine.rs:291`).
  2. So between detection and the actual halt, the faulting axis freezes (returns `None`) while **every other axis with a valid piece keeps stepping** — a transient single-axis desync, not a clean simultaneous stop.
  3. The real halt is foreground: the ~1 kHz `runtime_drain` task (`runtime_tick.c:352`) polls `last_error`, emits the `FaultEvent` (0x0082) one-shot, then calls Klipper `shutdown()` = `irq_disable()` + `longjmp` (`runtime_tick.c:421`). That kills the whole MCU. Latency: up to one drain period (≈1 ms) after the ISR detected the late piece.
  4. "Refuses further `PushPieces` until reset" is true only as a *consequence* of the global `shutdown()` killing the task loop — `push_pieces` itself has no fault guard (`engine.rs:237-256`). Recovery is a Klipper `FIRMWARE_RESTART`, not an engine reconfigure.
  5. **Only the detecting MCU shuts down.** There is no cross-MCU propagation in this scope — the other MCU keeps running its ring (cross-MCU stop is the deferred cancel work, §8). So a boundary underrun does not produce a clean coordinated stop; it produces one MCU in Klipper-shutdown and the other free-running until it too drains.

  The takeaway for this spec is unchanged in spirit — a late piece is catastrophic, not recoverable in-stream — but the design must not claim a clean engine-level "stop all steppers." The actual behavior is messier (transient desync + a ~1 ms-delayed whole-MCU Klipper shutdown, uncoordinated across MCUs), which only sharpens why §5.1's budgets must hold. Tightening the fault mechanism itself (set `RuntimeStatus::Fault`, coordinate cross-MCU) is **not** this spec's job; it is flagged here so the implementation plan doesn't lean on behavior the code doesn't have.
...
If the MCU drains its ring to the committed frontier, the axis idle-holds (§1.1). But the frontier sits at `t_decel_start − max_h` — **mid-cruise, nonzero velocity** — so the freeze is already an infinite-jerk stop. Worse, when the held-back ramp finally arrives its `start_time` is now in the MCU's past, which triggers `PIECE_START_IN_PAST` — and per §1.1 that is **not** a clean engine stop but a transient single-axis desync followed by a ~1 ms-delayed whole-MCU Klipper `shutdown()` (uncoordinated across MCUs), recoverable only by `FIRMWARE_RESTART`. Either way it is catastrophic, not a soft pickup. This is a halt-class invariant the design depends on, inherited from the streaming shaper (the old segment dispatch had it too). We do **not** fix it here — but the design relies on two separate budgets staying satisfied, and ring depth only covers one:
