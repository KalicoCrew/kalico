...
## Convolve diagnostic outcome (2026-04-26, commit `c7b482bd`)

Diagnostic test: degree-2 input with C⁰ kink at u=0.3 (m_x=2), constant kernel on [-0.1, 0.1]. Verified against numerical integration.

**Load-bearing case is sound.** At the kink images `u = u_x ± kernel_endpoint`, output multiplicity matches the convolution-continuity rule (`μ_y = m_x = 2`). Eval matches numerical integration to machine precision. Tiller A5.8's chord-error tolerance happens to land on the natural multiplicity for the kink-image regime — sound by accident, not by construction, but sound.

**Structural divergence at boundary cross-sums.** At `u = x.knots[boundary] + kernel_endpoint`, the spec's idealized convolution-continuity rule predicts `μ_y = 0` (no real continuity break, knot fully removable). The implementation produces `μ_y = 2` or `3` because the left/right polynomial pieces of `y` differ as polynomial expressions (by `(u − u_break)^k` terms that vanish at the join but not elsewhere), and Tiller A5.8 refuses to peel below this level — robust across tolerances from 1e-12 to 1e-2. **This is a fundamentally different removability question than Tiller asks** (functional equivalence vs polynomial-coefficient match), so tolerance tuning won't fix it.

**Deferred fix needed.** The output is correct in eval but bloated in representation (more interior knots than the natural-smoothness analysis predicts). Downstream Layer 3 work will pay a small cost in memory and evaluation steps for these spurious knots. The fix path is the convolve analog of `multiply`'s Fix 1: compute the convolution-continuity target multiplicities a priori from the cross-sum-of-input-knots structure (μ at cross-sum image of x's u_x = m_x; μ at boundary cross-sum = 0), then either bound knot removal by these targets OR construct the result with the target knot vector directly (the convolve analog of multiply Fix 2).

**Revisit when:** Layer 3 surfaces a concrete concern about output NURBS size from `convolve` (memory pressure, evaluation cost, or a downstream operation that's sensitive to extra interior knots). Not blocking smooth-shaper bake correctness today.
