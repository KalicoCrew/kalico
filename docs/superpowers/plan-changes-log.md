...
**Why:** Same-direction `_CLIENT_LINEAR_MOVE X=1 F=6000` × 3 reproducibly faulted on H7 hardware with `KALICO_ERR_STEP_BURST_EXCEEDED = -21` (`fault_code 65515`) on segment 3, with `prior_diag_summary_curve` showing `x_deg 9 x_cps 82 x_knots 92`. f32 De Boor evaluation on degree-9 NURBS with non-trivial absolute-position magnitudes suffers from catastrophic cancellation between control-point contributions — observed as one-tick position spikes ≥ 0.8 mm (= 64 steps × 1/80 mm/step). Alternating ±1 moves had stayed below the burst cap because endpoints cancel; same-direction accumulation pushed the De Boor walk into worse-conditioned regions.

The 2026-05-05 entry below documented this exact failure under sim and bumped the cap 16 → 64 as a deferred-fix workaround. That decision survived sim but bit on hardware once the host-side clock-sync wedge fix (this same session) exposed real motion attempts. Per CLAUDE.md's "uniform cubic Bézier across Layer 1/2/3/4" non-negotiable, the proper fix lives at the trajectory layer: keep dispatched curves at low degree so f32 evaluation is well-conditioned. The refit pass implements exactly that.

**Algorithm:** Sample the post-shape `ScalarNurbs<f64>` per axis, decompose to Bézier pieces via `extract_bezier_pieces`, fit cubic chain via merge-and-bisect with C¹ continuity at piece seams, recompose via `bezier_pieces_to_nurbs`. Adaptive split-and-retry loop bounded at 8 levels (≤ 256× input piece count worst case). Tolerance 0.1 µm matches CLAUDE.md's Goldapp 1991 cubic-fit tolerance for the Step-13 compat layer's arc reduction — sub-motor-resolution everywhere.

**Verification:** Same-direction X=+1×3 reproduction post-fix: klippy stays `ready`, no `kalico_fault` in log, `engine_status=2 (Running)` sustained through all three segments. All 61 trajectory-crate unit tests pass; the homing-diagnostic regression test passes. Burst cap restored to 16; `default_burst_cap_value` test pin updated. Note: physical step pulses are still unwired (no path from `shared.stepper_counts[i]` atomic counters to `step_pin` GPIO toggles on real hardware) — that's the actual "real step-output replacing trace-only stub" feature CLAUDE.md attributes to Step 7-B but the implementation today only covers the accumulator-math half. Refit closes the *fault*; physical motion remains a separate Step-7-B/7-D scope item.

**Stale claims this fix updates:**
- `docs/superpowers/specs/2026-04-30-step7a-layer3-trajectory-shaping-design.md` "Degree and piece-count budget" table (line 608): post-convolution row claimed degree-9 output was acceptable based purely on a compute-budget analysis (memory ✓ at 9.6 KB; FMA cycles ✓ at 360K/s on Cortex-M7). The analysis omitted f32 numerical-precision risk and assumed Horner evaluation; the runtime engine actually uses De Boor, which has a different cancellation profile. Spec table now needs a Stage 3b row showing post-refit cubic output (~10–50 cubic pieces per axis). Marked for spec update.
- The 2026-05-05 entry's deferred-fix follow-up ("Add a post-IS Hermite refit pass... Once landed, restore MAX_STEPS_PER_TICK_DEFAULT to 16") is now done.

**Evidence:**
- Diff: `rust/trajectory/src/refit.rs` (+200 new), `rust/trajectory/src/beta.rs` (+~20), `rust/trajectory/src/lib.rs` (+1), `rust/runtime/src/step.rs` (~12), `rust/runtime/tests/step_generation.rs` (1), `rust/nurbs/src/algebra.rs` (~10 — endpoint snap)
- Reproduction G-code (with permission gate per `feedback_no_gcode_without_permission` memory): `SET_KINEMATIC_POSITION X=100 Y=100 Z=10` then `_CLIENT_LINEAR_MOVE X=1 F=6000` × 3. Pre-fix: deterministic fault on segment 3 with `fault_code 65515 segment_id 3`. Post-fix: all three complete cleanly.

**Lesson:** Compute budget review is not precision review. The Step-7A spec verifier confirmed "math is correct" for the convolution chain (degree, piece count, exactness) and computed cycle/memory budget for evaluation, but the verifier scope did not include numerical conditioning of the f32 evaluator on the produced output. CLAUDE.md's "uniform cubic Bézier across Layer 1/2/3/4" mandate exists precisely to bound this risk; the spec author read the mandate as constraining each layer's *input* rather than its *output*, and the convolve-stage's output-degree-explosion was treated as legitimate within that frame. Pattern to flag: when a spec accepts a degree increase on the path to a numerically-constrained evaluator, require an explicit precision-on-target check, not just a compute-fits check.

---

### 2026-05-10 — Bridge-mode periodic clock-sync driver closes the host-side wedge

**What:** Added a per-MCU `clock-sync-mcu-{N}` thread in `rust/motion-bridge/src/bridge.rs` that issues `runtime_clock_sync_request` every 500 ms via the same kalico-native path the ARMING flow uses (`stream.rs::arm_all_mcus`), feeds responses into a per-thread `ClockSyncEstimator`, and pushes each fresh RTT-corrected sample into `PassthroughRouter` via the new `set_clock_est_from_sample(mcu, freq, host_send: Instant, mcu_at_send: u64)` accessor (`rust/kalico-host-rt/src/passthrough_queue/router.rs`). Spawned at the end of `attach_serial` for any MCU that passes `kalico_identify`; stopped via `Arc<AtomicBool>` and joined in `release_mcu`. ~177 added lines in `bridge.rs` + 23 in `router.rs`. No firmware change.

**Why:** Reproducible wedge symptom — `_CLIENT_LINEAR_MOVE X=1` / `X=-1` alternation at F=6000 deterministically triggered `slot pool exhausted (16/16); awaiting kalico_credit_freed retirement events` after ~10 submits, followed by `MCU 'mcu' shutdown: Command request` and 5 s `bridge_call: transport timed out`. Initial diagnosis suspected MCU-side CPU starvation (per the `diagnosing-h7-mcu-wedge` skill's common-findings table), but `prior_diag` showed a healthy MCU: TIM5 firing at ~20 kHz, foreground gaps <100 ms, engine reaching `Running`, queue depth growing 1→8 with `bnd_iters=0` (boundary loop never fired despite `now − tstart` exceeding `duration` by 7×). Instrumented the bridge dispatch closure with `[wedge-host]` eprintln and discovered the host was sending segments with `t_start ≈ 30 s in the MCU's future`. Root cause: `compute_ack_clock = last_clock + (now − offset)·freq` was extrapolating from an anchor frozen at klippy's connect-time `set_clock_est`, drifting linearly into the future for the entire klipper session.

The bug lives in the bridge-mode contract: `klippy/serialhdl.py:raw_send` is a no-op for bridge MCUs (line 552: "Clock sync is driven by the bridge via set_clock_est callbacks"). Klippy's `clocksync._get_clock_event` therefore drops `get_clock` on the floor every second; the MCU never replies; klippy's `_handle_clock` never runs after `connect()`; the registered `_bridge_clock_est_cb` fires exactly once (the post-connect refresh in `set_clock_est_callback`) and then is silent forever. The aspirational comment in `serialhdl.py` was never backed by an implementation — `add_piggyback_sample` exists in `clock_sync.rs` but is called nowhere outside tests, and the dedicated `kalico_clock_sync_request` runs only at `arm_all_mcus`. This driver is what the comment was promising.

**Verification:** Before fix, 9 submits filled the host's slot pool and klippy command-reset the MCU; `prior_diag` showed `cur_id 1 ret_id 0 bnd_iters 0` (engine stuck on segment 1, never advanced) with `tstart_hi 4 tstart_lo 3008523493 = 20.19B cycles ≈ 38 s` vs `now ≈ 4.5B cycles ≈ 8.7 s` — segment scheduled 30 s in the future. After fix, 16 sequential submits all return 200; `prior_diag` shows `cur_id 16 acc_id 16 ret_id 16 bnd_iters 16 status=Drained` (all segments consumed via the boundary loop). A separate motion-side fault (`KALICO_ERR_STEP_BURST_EXCEEDED = -21`, surfaced as `fault_code=65515`) appears under sustained heavy load — that's a real motion-side issue previously masked by the wedge, not regressed by this fix.

**Stale claims this fix exposed (updated in same session):**
- `.claude/skills/diagnosing-h7-mcu-wedge/SKILL.md` — common-findings table had no entry for the host-side wedge class; the `rcc 21102592 / SFTRSTF / prior=0` worked example pointed exclusively at MCU-side causes (lockup escalation, command_reset). Added a "Host-side wedges" section + new common-findings row + updated the worked example to enumerate the three sub-cases.
- `docs/superpowers/specs/2026-04-28-step6-comm-protocol-and-sim-fixes-design.md` §5.3, §12.3, §13.2 — designed `kalico_status_v6` to carry `mcu_clock_now` per frame and feed every regression sample. The host-side `StatusEvent` (`rust/kalico-host-rt/src/host_io/runtime_events.rs:20`) has no `mcu_clock_now` field (payload is 18 bytes of status/queue/segment/fault metadata only), and the firmware emit (`src/kalico_dispatch.c:469`) doesn't include MCU clock either. The piggyback path was specced but never wired. Marked the spec sections as deferred; the periodic dedicated round-trip from this fix supersedes the piggyback design for now.
- `docs/superpowers/plans/2026-05-04-bridge-mode-path-a-completion.md:195` — risk register asserted "clock-sync regression hooks (`_bridge_clock_est_cb`) already exist; verify they fire post-Phase-1." The hooks exist but never fire periodically in bridge mode — the verification this plan called for would have surfaced the bug we just fixed. Left as historical record (closed plan); this changelog entry is the public correction.

**Evidence:**
- Diff: `rust/kalico-host-rt/src/passthrough_queue/router.rs` (+23), `rust/motion-bridge/src/bridge.rs` (+177)
- Reproduction G-code (with permission gate, see `feedback_no_gcode_without_permission` memory): `SET_KINEMATIC_POSITION X=100 Y=100 Z=10` then alternating `_CLIENT_LINEAR_MOVE X=1 F=6000` / `X=-1 F=6000`. Pre-fix: 9/10 succeed then deterministic crash. Post-fix: 16/16 succeed.
- Live counters captured during diagnosis (boundary iters, current/accepted/retired segment ids, now/tstart hi+lo) — all instrumentation removed before commit.

**Lesson:** When the wedge symptom matches the MCU-starvation skill's flowchart but the MCU's own snapshot says "I'm healthy," the wedge is host-side. The skill's common-findings table was 100 % MCU-side; an aspirational comment in `serialhdl.py` ("Clock sync is driven by the bridge via set_clock_est callbacks") had no implementation backing it. Both the symptom catalog and the comment treated the broken state as the working state for ~6 months. Pattern to flag: any "X already exists; just verify" risk-register entry warrants verifying *behavior*, not *presence of the hook*.
...
To preserve mainline's host-side enforcement of "Must home axis first," `BridgeKinematics` now carries a `self.limits = [(1.0, -1.0)] * 3` (the unhomed sentinel) instead of a `homed_axes: set` and ports the `_check_endstops` / `check_move` body verbatim from upstream `klippy/kinematics/cartesian.py` (cartesian and corexy share the same body). `set_position(newpos, homing_axes=…)` updates `self.limits[axis] = rail.get_range()` for each homed axis. `clear_homing_state` and the `motor_off` event reset back to the sentinel. `MotionToolhead.move()` now constructs a `Move` and runs `kin.check_move` + `extruder.check_move` before submitting to the bridge, mirroring `ToolHead.move`.

Orthogonal fix landed in the same change: `klippy/mcu.py:_home_wait_bridge` no longer pre-sets `_dispatch._reason = REASON_COMMS_TIMEOUT` before calling `dispatch.stop()` on the silence-backstop path. The pre-set short-circuited the disarm in `BridgeTriggerDispatch.stop`, leaving the MCU runtime in `Armed` state and rejecting every subsequent G28 with `ArmStatus::Rejected` (Busy). `stop()` now sees `_reason is None` and sends `runtime_disarm_endstop` over the wire before the error propagates.

**Why:** First live G28 X attempt against trident.local (sota-motion HEAD `9c5dd944a`) hung for 5.66 s and aborted with `Homing wait: MCU silent past expected end-time + 1.0s (no trip event, no credit-freed for homing segment)`. Diagnosis (klippy.log lines 675299–675383) showed `[bridge-trace] endstop_arm status=0` and `drip_move dispatched` succeed, the bridge planner thread shapes the segment and `wait_moves()` returns, yet the MCU's `kalico_status_v6` heartbeat reports `engine_status=0, current_segment_id=0` for the entire backstop window — i.e. the segment was queued but never dequeued. Root cause: `engine.rs:325–339` with `homed=false` and `stream_open=false` silently returned `Ok(())` on every tick before reaching the queue check, so segments accumulated forever. The fix paths attempted in `motion_toolhead.py:487-491` (sim-only `set_homed_state(True)` whitelist) and `homing.py:_notify_bridge_homed` (post-G28-success notification) are both symptoms of the gate being the wrong abstraction: "homed" is a host-side kinematics state (which axes have known position), not a runtime motion-execution state, and a gate that blocks all motion when not-homed cannot, by construction, allow the homing motion that establishes homed-ness without an out-of-band tag the gate was never given. Mainline kalico/klipper has no MCU-side homed gate at all — kinematic validation lives entirely on the host (range-clamp via `cartesian/corexy.check_move` against per-axis `limits` with the `(1.0, -1.0)` "unhomed" sentinel), endstop-based runtime safety lives in `endstop::tick`. Restoring that split is the correct architectural answer; defense-in-depth at the wrong layer was actively breaking the system.

**Evidence:** Live failure log `~dderg@trident.local:/printer_data/logs/klippy.log` lines 675299–675475 (Step 7-D bring-up session 22:20:33). Pre-existing motion-bridge `tests/sim_motion.rs` breakage confirmed unrelated by `git stash` + `cargo check -p motion-bridge --tests` against unmodified HEAD (same 8 errors). Runtime test suite passes after gate removal. `cargo check --workspace --lib` clean. C headers regenerated; `grep -rn 'set_homed\|NotHomed\|KALICO_ERR_NOT_HOMED' rust/ klippy/ src/ tools/` clean apart from unrelated trad_rack `SelectorNotHomedError` and `homing.py:set_homed_position`.

**Lesson:** When the upstream-mainline equivalent of a feature is "we don't have one," reconsider whether the new feature actually solves a problem the host can't already solve at the right layer. The Step 7-B homed gate was added as defense-in-depth, but the layer it lived at (MCU runtime, segment-execution gate) had no notion of axes or homing — it could only enforce a binary at-rest/in-motion split that the host's kinematics layer already enforces with axis-level granularity. Defense-in-depth that encodes a concept the layer can't represent isn't defense; it's a deadlock waiting for the right test.
