...
/// from `CappedAtMaxSweeps`, which signals genuine MAX_SWEEPS exhaustion
...
- **Dirty-on-non-success can spin to MAX_SWEEPS** — round-2 stance. kalico-plan-reviewer ruled it acceptable (bounded by MAX_SWEEPS × dirty count, surfaces via `joining_status` field); Codex called it MAJOR. Both had merit. **Note for narrative consistency:** this stance was REVISED in round 3 — the dirty-spin issue *was* fixed via Codex's early-bail recommendation, verified by kalico-verifier. Post-round-4 it surfaces via the dedicated `JoiningStatus::StalledOnInfeasibleSegment { last_dirty_count }` variant, NOT via `CappedAtMaxSweeps`. The "keeping current behavior" framing in this paragraph reflects the round-2 decision and is preserved for changelog continuity; the actual current behavior is the round-3-corrected early-bail.
